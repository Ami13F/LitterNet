    ...
    def create_network(self):
        ...
        for i, block in enumerate(self.blocks[1:]):
            ...
            if block["type"] == "convolutional":
                ...
                inputs = MaxPool2D(size, strides=
                    stride, padding=padd)(inputs)
            ...
            # If block is route layer
                ...
                if len(ind_backward) > 1:
                    ...
                    inputs = tf.concat(
                        [outputs[i + start], outputs[end]], axis=-1,
            ...
            # Skip layers
                ...
                if outputs[step].shape[-1] < out_channels:
                    padd_val = (out_channels - outputs[step].shape[-1])
                    padding = tf.constant(
                        [[0, 0], [0, 0], [0, 0], [0, padd_val]])
                    temp = tf.pad(outputs[step], padding, "CONSTANT")
                elif outputs[step].shape[-1] > out_channels:
                    temp = outputs[step][:, :, :, :out_channels]
                else:
                    temp = outputs[step]
                inputs = tf.math.add(outputs[i - 1], temp)
